...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private static final float SIGNIFICANT_MOVE_THRESHOLD = 0.33f;//0.4f;

    // The following constants need to be scaled based on density. The scaled versions will be
    // assigned to the corresponding member variables below.
    private static final int FLING_THRESHOLD_VELOCITY = 300;//500;
    private static final int MIN_SNAP_VELOCITY = 1500;
    private static final int MIN_FLING_VELOCITY = 250;
    //protected static final float OPTIMIZE_REACTION_SCALE = 0.5f; 
    ...
    protected long mCurrentScreenId;
    protected int mNextPage = INVALID_PAGE;
    protected int mMaxScrollX;
    protected Scroller mScroller;
    protected VelocityTracker mVelocityTracker;
    protected DesktopIndicator mIndicator;
    ...
    // Begin, added by zhumeiquan, for swipe gesture, 20120918
    protected final static int TOUCH_SWIPE_DOWN_GESTURE = 4;
    protected final static int TOUCH_SWIPE_UP_GESTURE = 5;
    ...
    private float mTouchReactionScale = 0.5f;
    ...
    public interface OverScrollListener {
        void updateOverScrollAmount(int screenScroll);
        boolean onOverScrollUp(int nearestPage);
    ...
    protected void init() {
        ...
        mTouchReactionScale = getResources().getFraction(R.fraction.config_touchReactionScale, 1, 1);
        mPagingTouchSlop =(int)(configuration.getScaledPagingTouchSlop() * mTouchReactionScale);
    ...
    protected void initScroller(Interpolator interpolator) {
    ...
    //yixiao add for piflow 2015.1.11 begin
    long getRunPage() {
        int currentPage = getCurrentPage();
        return (currentPage >= 0 && currentPage < getPageCount()) ? LauncherModel.getScreenIdByIndex(currentPage):-1;
    }
    //yixiao add for piflow 2015.1.11 end
...
//        }
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            // Begin, added by zhumeiquan, 20120926
            float scale = 1.0f;
            if (mIndicator != null) {
                mIndicator.indicate(mScroller.getCurrX() / (getChildCount() * getWidth() *  scale));
            }
            // End
            invalidate();
            return true;
            ...
            // Begin, added by zhumeiquan, 20120926
            if (mIndicator != null) {
                mIndicator.fullIndicate(mCurrentPage);
            }
            // End
    ...
    protected int getChildOffset(int index) {
        ...
        if (childOffsets != null && index >= childOffsets.length) {
            index = childOffsets.length - 1;
        }
    ...
    protected int getScaledMeasuredWidth(View child) {
        ...
        //yixiao add #70188 2015.2.13 begin
        if(child==null){
            return 0;
        }
        //yixiao add #70188 2015.2.13 begin
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        // Begin, added for stop multitouch, do not support more than one point, 20130914
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mActivePointerId != INVALID_POINTER) {
                    ...
                    mActionMoveCount ++;
                    if (mTouchState == TOUCH_STATE_SCROLLING && mActionMoveCount < 5) {
                    	// the mLastMotionX will be reset in determineScrollingStart.
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        final int touchSlop = Math.round(touchSlopScale * mTouchSlop * mTouchReactionScale);
        boolean xPaged = xDiff > mPagingTouchSlop;
        boolean xMoved = xDiff > touchSlop;
        boolean yMoved = yDiff > touchSlop;

        if (xMoved || xPaged || yMoved) {
            if ((mUsePagingTouchSlop ? xPaged : xMoved) && (yDiff < xDiff)) {
         ...
         // Begin, added by zhumeiquan, for swipe gesture, 20120919
            else if (yDiff > (touchSlop * 2)) {
                boolean mTouchedScrollableWidget = isWidgetAtLocationScrollable((int)mLastMotionX, (int)mLastMotionY);
                if (!mTouchedScrollableWidget) {
				//zwsun@letek.com 20150108 start
                    if (mLauncher.getWorkspace().getOpenFolder() == null && !mLauncher.isPiflowPage()) {
                        if (y - mLastMotionY > 0) {
    ...
    // Begin, return true if widget at this position is scrollable. zhumeiquan, 20140421
    public boolean isWidgetAtLocationScrollable(int x, int y) {
        ...
        CellLayout curScreen = (CellLayout) child;
        int[] pos = new int[2];
        curScreen.pointToCellExact(x, y, pos);
        int count = curScreen.getShortcutsAndWidgets().getChildCount();
        for (int i = 0; i < count; i++) {
            View cell = curScreen.getShortcutsAndWidgets().getChildAt(i);
            if (cell != null && cell instanceof LauncherAppWidgetHostView) {
                CellLayout.LayoutParams lp = (CellLayout.LayoutParams) cell.getLayoutParams();
                if (pos[0] >= lp.cellX && pos[0] < lp.cellX + lp.cellHSpan
                    && pos[1] >= lp.cellY && pos[1] < lp.cellY + lp.cellVSpan) {
                    return true;
                }
            }
        }
    ...
    protected float getScrollProgress(int screenScroll, View v, int page) {
        ...
        if (isSupportCycleSlidingScreen() && getChildCount() > 1) {
            if(screenScroll > mMaxScrollX && page == 0){
                delta = delta - mMaxScrollX - getScaledMeasuredWidth(v);
            }else if(screenScroll < 0 && page == getChildCount() -1){
                delta = delta + mMaxScrollX + getScaledMeasuredWidth(v);
            }
        }
        ...
        if (DEBUG) {
            Log.d(TAG, "page=" + page + "screenScroll=" + screenScroll + ",delta=" + delta
                    + "scrollProgress=" + scrollProgress + ",screenCenter=" + screenCenter);
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        //lqwang-PR61973-modify begin
        if (ev.getPointerCount() > 2) {
            snapToDestination();
            mTouchState = TOUCH_STATE_REST;
            return super.onTouchEvent(ev);
        }
        //lqwang-PR61973-modify end
        // Skip touch handling if there are no pages to swipe
        ...
        case MotionEvent.ACTION_MOVE:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                final int pointerIndex = ev.findPointerIndex(mActivePointerId);
                // Begin, added by zhumeiquan, for the bug SW2 #39699, maybe crashed at 4.0 or later
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (((isSignificantMove && deltaX > 0 && !isFling) ||
                    ...
                    // Modify by Fan.Yang to cycle sliding screen
                    if(mCurrentPage > 0){
                        //lqwang - edit mode can not move to piflow - modify begin
                        finalPage = returnToOriginalPage || isPiflowInEditMode() && mCurrentPage == Constants.FULL_WIDGET_COUNT ? mCurrentPage : mCurrentPage - 1;
                        //lqwang - edit mode can not move to piflow - modify end
            ...
            // Begin, added by zhumeiquan, for swipe gesture, 20120919
                ...
                if (!mLauncher.inPreviewMode() && !mLauncher.isFloating() && !mLauncher.isHiddenOpened() && !mLauncher.isEditMode()) {
                    if (PreferencesProvider.getSwipeUpDown(getContext())) {
                        ReflUtils.showNotifications(mLauncher);
                    }
                }
    ...
    protected int getChildWidth(int index) {
        ...
        final int measuredWidth = v == null ? 0 : v.getMeasuredWidth();
        final int minWidth = mMinimumWidth;
    ...
    protected void snapToDestination() {
        ...
        //lqwang - edit mode can not move to piflow - modify begin
        if(isPiflowInEditMode() && nearestPage <= Constants.FULL_WIDGET_COUNT - 1){
            nearestPage = Math.min(Constants.FULL_WIDGET_COUNT,getChildCount() - 1);
        }
        //lqwang - edit mode can not move to piflow - modify end
        snapToPage(nearestPage, PAGE_SNAP_ANIMATION_DURATION);
    }
    ...
    private void doScroll(MotionEvent ev) {
        ...
        if (Math.abs(deltaX) >= 1.0f) {
            /// M: modify by Fan.Yang to cycle sliding screen.
            if (isSupportCycleSlidingScreen()) {
                if (deltaX < 0) {
                    if (mTouchX > 0) {

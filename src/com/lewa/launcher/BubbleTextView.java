...
public class BubbleTextView extends TextView {
    ...
    private Drawable new_hint_dr;

    //for new app tip-lqwang-add begin
    public boolean isNewAdded = false;
    private int new_hint_size;
    private final int new_hint_padding = 3;
    private boolean isDragging = false;
    private int hint_width;
    private String occupied = "    ";
    //for new app tip-lqwang-add end
    private boolean showNewIndicator = true;
...
//        mLongPressHelper = new CheckLongPressHelper(this);
       ...
       // setShadowLayer(SHADOW_LARGE_RADIUS, 0.0f, SHADOW_Y_OFFSET, SHADOW_LARGE_COLOUR);
        new_hint_dr = res.getDrawable(R.drawable.new_hint);
        new_hint_size = res.getDimensionPixelSize(R.dimen.new_hint_size);
        new_hint_dr.setBounds(0, 0, new_hint_size, new_hint_size);
        hint_width = new_hint_dr.getBounds().width() + new_hint_padding;
    }
    ...
    public void applyFromShortcutInfo(ShortcutInfo info, IconCache iconCache) {
        ...
        if (cn != null) {
            String path = IconCustomizer.getFancyIconRelativePath(cn.getPackageName(), cn.getClassName());
            if (path != null) {
                d = AppIconsHelper.getIconDrawable(getContext(), path);
            }
        }
        if (d == null) {
            d = new FastBitmapDrawable(info.getIcon(iconCache));
        }
        mIcon = d;
        setCompoundDrawablesWithIntrinsicBounds(null,  d, null, null);
        mShowPressed = false;
        if (info != null && info.title != null) {
            String title = Utilities.convertStr(getContext(), info.title.toString());
            setText(title);
        }
        setTag(info);
    }
    ...
    private void drawWithPadding(Canvas destCanvas, int padding) {
        ...
        // something wrong when getExtendedPaddingTop(), add protect.
        if (getLayout() == null) {
            android.util.Log.e("BubbleTextView", "ERROR! getlayout==null when drawWithPadding()");
            return;
        }
        // end
    ...
    protected void onDraw(Canvas canvas) {
        //for new app tip-lqwang-add begin
        if(isNewAdded && !isDragging && getLayout() != null && isShowNewIndicator()){
            canvas.save();
            int extendedPaddingTop = getExtendedPaddingTop();
            canvas.translate(getLayout().getLineLeft(0),extendedPaddingTop+(getLayout().getHeight()-new_hint_dr.getBounds().height())/2);
            new_hint_dr.draw(canvas);
            canvas.restore();
        }
        //for new app tip-lqwang-add end
        super.onDraw(canvas);
    ...
    public void setShowNewIndicator(boolean showNewIndicator) {
        this.showNewIndicator = showNewIndicator;
    }

    public void setNewAdded(boolean newAdded) {
        if(newAdded){
            setText(occupied+getText().toString().trim()+occupied);
        }else if(!newAdded){
            setText(getText().toString().trim());
        }
        this.isNewAdded = newAdded;
        invalidate();
    }
...
//                mLongPressHelper.cancelLongPress();
    ...
    Drawable createPressedDrawable() {
        final Bitmap b = Bitmap.createBitmap(
                mIcon.getIntrinsicWidth(), mIcon.getIntrinsicHeight(),Bitmap.Config.ARGB_8888);
        mTempCanvas.setBitmap(b);
        mIcon.draw(mTempCanvas);
        mTempCanvas.drawColor(0x66000000, PorterDuff.Mode.SRC_ATOP);
        mTempCanvas.setBitmap(null);
        return new FastBitmapDrawable(b);
    }
    
    void setIconPressed(boolean pressed) {
        if (pressed) {
            if (!mShowPressed && mIcon != null) {
                if (mIcon instanceof FancyDrawable) {
                    Drawable d = createPressedDrawable();
                    setCompoundDrawablesWithIntrinsicBounds(null,  d, null, null);
                } else {
                    mIcon.setColorFilter(0x66000000, PorterDuff.Mode.SRC_ATOP);
                    setCompoundDrawablesWithIntrinsicBounds(null,  mIcon, null, null);
                }
                invalidate();
                mShowPressed = true;
            }

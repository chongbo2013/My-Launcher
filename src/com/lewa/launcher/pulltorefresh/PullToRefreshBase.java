 ...
 *******************************************************************************/
package com.lewa.launcher.pulltorefresh;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    protected void onRefreshing(final boolean doScroll) {
        ...
        if (doScroll && !stayPosOnRefreshing()) {
    ...
    protected final void setHeaderScroll(int value) {
        ...
        switch (getPullToRefreshScrollDirection()) {
            case VERTICAL:
                scrollViewTo(0, value);
                break;
            case HORIZONTAL:
                scrollViewTo(value, 0);
                break;
        }
    }
    ...
    protected void updateUIForMode() {
        ...
        if(hideFooterView()){
            mFooterLayout.setAlpha(0);
        }

        if(hideHeaderView()){
            mHeaderLayout.setAlpha(0);
        }else{
    ...
    private void pullEvent() {
        ...
        if(Launcher.enterPreview){
            return;
        }
        //yixiao add #70114 2015.3.17
        Launcher.hotsetMove = true;
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            float scale = Math.abs(newScrollValue) / (float) itemDimension; //当前移动距离/refresh view的宽度（高度）
            switch (mCurrentMode) {
                case PULL_FROM_END:
            ...
            if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {//移动距离小于refresh view的宽度（高度），pull to refresh
                setState(State.PULL_TO_REFRESH);
            } else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {//移动距离小于refresh view的宽度（高度），release to refresh
    ...
    private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
        ...
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                oldScrollValue = getViewScrollX();
                break;
            case VERTICAL:
            default:
                oldScrollValue = getViewScrollY();
                break;
        }
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mContinueRunning && mScrollToY != mCurrentY) {
                ViewCompat.postOnAnimation(getScrollView()!=null ? getScrollView() : PullToRefreshBase.this, this);
            } else {
                if (null != mListener) {

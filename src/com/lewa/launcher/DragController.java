...
public class DragController {
    ...
    private static final long[] VIBRAT_PATTERN = { 0, 1, 20, 21};
    ...
    public void startDrag(View v, DragSource source, Object dragInfo, int dragAction,
        ...
        if (mLauncher.isWorkspaceLocked()
                || !LauncherModel.isItemInfoSynced(dragInfo)) {
            return;
        }
        ...
        startDrag(b, dragLayerX, dragLayerY, source, dragInfo, dragAction, null, dragRegion, 1.0f);
        if(!b.isRecycled()){
            b.recycle();
        }
    ...
    public void startDrag(View v, Bitmap bmp, DragSource source, Object dragInfo, int dragAction,
        ...
        if (mLauncher.isWorkspaceLocked() || !LauncherModel.isItemInfoSynced(dragInfo)) {
            return;
        }
        mLauncher.getDragLayer().getLocationInDragLayer(v, loc);
    ...
    public void startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        mVibrator.vibrate(VIBRAT_PATTERN, -1);
        ...
        if (mDragObject != null) {
            ...
            if (rawDragInfo instanceof ShortcutInfo) {
                ...
                for (ApplicationInfo info : apps) {
                    ...
                    if (dragInfo != null &&
                        dragInfo.intent != null && dragInfo.intent.getComponent() != null && info != null && info.intent != null) {
                        boolean isSamePackage = dragInfo.intent.getComponent().equals(info.intent.getComponent());
                        if (isSamePackage) {
                            cancelDrag();
                            return;
                        }
    ...
    private void handleMoveEvent(int x, int y) {
        ...
        // Begin, added for floating
        if (mLauncher.isFloating()) {
            return;
        }
        // End

        if (x < mScrollView.getLeftSide() + mScrollZone) {
            scrollLeft(x, y, delay);
        } else if (x > mScrollView.getRightSide() - mScrollZone) {
            scrollRight(x, y, delay);
        } else {
            clearScrollRunnable();
        }
    }

    private void scrollLeft(int x, int y, int delay){
    ...
    private void scrollRight(int x, int y, int delay){
        if (mScrollState == SCROLL_OUTSIDE_ZONE) {
            mScrollState = SCROLL_WAITING_IN_ZONE;
	...
	//yixiao add #953261
        if(Launcher.enterPreview && ev.getPointerCount()>1){
            return false;
        }
        // Update the velocity tracker
        acquireVelocityTrackerAndAddMovement(ev);
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            mMotionDownX = dragLayerX;
            mMotionDownY = dragLayerY;
            if ((dragLayerX < mScrollView.getLeftSide() + mScrollZone) ||
                    (dragLayerX > mScrollView.getRightSide() - mScrollZone)) {
                mScrollState = SCROLL_WAITING_IN_ZONE;
                //#62078 Modify by fan.yang, increase delay from SCROLL_DELAY to RESCROLL_DELAY
                mHandler.postDelayed(mScrollRunnable, RESCROLL_DELAY);
            } else {
                mScrollState = SCROLL_OUTSIDE_ZONE;
            }
            break;
    ...
    private void drop(float x, float y) {
        ...
        if (dropTarget != null ) {
            ...
            if (dropTarget.acceptDrop(mDragObject) && LauncherModel.isItemInfoSynced(mDragObject.dragInfo)) {
    ...
    private DropTarget findDropTarget(int x, int y, int[] dropCoordinates) {
        ...
        final Folder folder = mLauncher.getWorkspace().getOpenFolder();
        final boolean isFloating = mLauncher.isFloating();
        for (int i=count-1; i>=0; i--) {
            DropTarget target = dropTargets.get(i);
            if (!target.isDropEnabled())
                continue;

            if (folder != null/* && !isFloating*/) {
                int top = folder.folderY;
                int bottom = isFloating ? top + folder.getFolderHeight() :
                        top + folder.getCurrentHeight();
                if (target instanceof FolderLayout && (y > bottom)) {
                    continue;
                }
            }
            target.getHitRect(r);
    ...
    private class ScrollRunnable implements Runnable {
        ...
        public void run() {
            if (mDragScroller != null) {
                ...
                if (isDragging()) {
                    /*
                    * #62527 delete by Fan.Yang ，文件夹图标跨屏拖动
                    * 在findDropTarget之后mDragObject.x和mDragObject.y是相对DropTarget的坐标
                    * 在这个时候forceMoveEvent,会造成拖动图标的位置出现错误，因为handleMoveEvent的坐标参数是
                    * 相对与DragLayer的坐标系的，Workspace上的拖拽滚动屏幕不会出现这个因为findDropTarget计算
                    * 出的coordinates不是相对坐标，费解！是bug？
                    * */

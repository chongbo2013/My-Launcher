...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        //Start added by weihong, #56743, Don't allow third parties to add application shortcuts
        //we can't do this , because all app will not create shortcut #PR 64172
//        if (true) {
//            return;
//        }
        // End
    ...
    private static void processInstallShortcut(Context context,
        ...
        boolean found = false;
        // the intent in database has flags, so add.
        ...
        if (intent.getAction().equals(Intent.ACTION_MAIN)) {
            ...
            launcherIntent.setComponent(intent.getComponent());
        } 
        // end
        synchronized (app) {
            final ArrayList<ItemInfo> items = LauncherModel.getItemsInLocalCoordinates(context);
            final boolean exists = LauncherModel.shortcutFromThridAppExists(context, name, intent)
                                   || LauncherModel.shortcutFromThridAppExists(context, name, launcherIntent);//lqwang - pr938336 - modify

            // Try adding to the workspace screens incrementally, starting at the default or center
            // screen and alternating between +1, -1, +2, -2, etc. (using ~ ceil(i/2f)*(-1)^(i-1))
            final int screen = PreferencesProvider.DEFAULT_SCREEN;
            int workspaceCnt = LauncherModel.getScreenCnt();
            for (int i = 0; i < (2 * workspaceCnt) + 1 && !found; ++i) {
                int si = screen + (int) ((i / 2f) + 0.5f) * ((i % 2 == 1) ? 1 : -1);
                if (0 <= si && si < workspaceCnt) {
    ...
    private static boolean installShortcut(Context context, Intent data, ArrayList<ItemInfo> items,
        ...
        if (findEmptyCell(context, items, tmpCoordinates, screen)) {
            if (intent != null) {
                ...
                // By default, we allow for duplicate entries (located in different places)
                boolean duplicate = data.getBooleanExtra(Launcher.EXTRA_SHORTCUT_DUPLICATE, false);
                boolean userCreated = data.getBooleanExtra(Launcher.EXTRA_SHORTCUT_USER_CREATED, false);
                if (!userCreated) { // if the icon is not created by user, disable the duplication.
                    duplicate = false;
                }

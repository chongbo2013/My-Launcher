...
public class DragLayer extends FrameLayout
        implements ViewGroup.OnHierarchyChangeListener, DragController.ScrollView {
    ...
    // Begin, added by zhumeiquan
    private boolean isEventOverAppRecommend(MotionEvent ev) {
        //lqwang - remove smart sort and recommend - modify begin
        if (!mLauncher.isRecommendOpened() || !PreferencesProvider.isRecommendOn(mLauncher)) {
            return false;
        }
        //lqwang - remove smart sort and recommend - modify end

        getDescendantRectRelativeToSelf(mLauncher.getAppRecommend(), mHitRect);
    ...
    private boolean isEventOverFloatingRest(MotionEvent ev) {
        if (!mLauncher.isFloating()) {
            return false;
        }
        Rect rect = mLauncher.getFloating().getRestSeatRect();
    ...
    private boolean handleTouchDown(MotionEvent ev, boolean intercept) {
        ...
        if (currentFolder != null && intercept) {
            if (!currentFolder.isOpen()) {
                mFolderScroller = null;
                // added by weihong, #60585
                return true;
            }
            ...
            if (currentFolder.handleTouchDown(ev)) {
                return true;
            } else if (isEventOverAppRecommend(ev) || isEventOverFloatingRest(ev)) {
                return false;
            }

            getDescendantRectRelativeToSelf(currentFolder, hitRect);
            if (!isEventOverFolder(currentFolder, ev)) {
                mFolderScroller = currentFolder.getFolderLayout();
                return true;
            }
    ...
    private boolean handleTouchUp(MotionEvent ev) {
        Folder currentFolder = mLauncher.getWorkspace().getOpenFolder();
        if (currentFolder != null && currentFolder.isOpen()) {
            if (isEventOverAppRecommend(ev) || isEventOverFloatingRest(ev)
                    || !currentFolder.isOpen() || mDragController.isDragging()) {
                return false;
            }
            if (!currentFolder.isEventOverFolder(ev)
                    && !isEventOverFolderTextRegion(currentFolder, ev)) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        // Begin, added by zhumeiquan, 20120918
        ...
        // Begin, added for hidding apps, 20131224
        else if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            if (ev.getPointerCount() == THREE_POINT) {
                yDiff = ev.getY() - mLastDownY;
                if (Math.abs(yDiff) > SWIPE_THRESHOLD) {
                    if (yDiff < 0) {
                        mLauncher.openHiddenApps();
                    } else {
                        mLauncher.closeHiddenApps();
                    }
                    return true;
                }
            }
        ...
        if (ev.getAction() == MotionEvent.ACTION_UP) {
            if (Math.abs(ev.getX() - mLastDownX) < 15 && handleTouchUp(ev)) {
                return true;
            }
        }
    ...
    private FolderScrollCallbacks mFolderScroller;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mFolderScroller != null && mDragController != null && !mDragController.isDragging()) {
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    mFolderScroller.onActionDown(ev);
                    break;
                case MotionEvent.ACTION_MOVE:
                    mFolderScroller.onActionMove(ev);
                    break;

                case MotionEvent.ACTION_CANCEL:
                case MotionEvent.ACTION_UP:
                    mFolderScroller.onActionUp(ev);
            }
            return true;
        }
    ...
    public void animateViewIntoPosition(DragView dragView, final View child, int duration,
        ...
        Runnable onCompleteRunnable = new Runnable() {
            public void run() {
                ...
                float scale = getCellScale(child);
    ...
    public float getCellScale(View view) {
        boolean isFloated = mLauncher.getFloating().isFloatedCell(view);
        if (isFloated) {
            return 1f;
        }
        if (mLauncher.isFloating()) {
            return FloatingLayer.FLOATING_SCALE;
    ...
    public boolean isDropAnimRunning() {
        return mDropAnim != null && mDropAnim.isRunning();
    }
    ...
    private class ScaleDetectorListener implements ScaleGestureDetector.OnScaleGestureListener {
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (mLauncher == null || mLauncher.getWorkspace() == null || mLauncher.getWorkspace().getCurrentCellLayout() == null) {
                return false;
            }
            boolean flag;
            if (f < 0.9F) {
			    //yixiao add #70114 2015.3.17
                if(!Launcher.hotsetMove){
                    mLauncher.enterPreviews();
                }

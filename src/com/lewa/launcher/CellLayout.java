...
public class CellLayout extends ViewGroup implements View.OnClickListener {
    ...
    private int mWorkspacePaddingBottom;
    //zwsun@letek.com 20150108 start
    private boolean isPifLowPage = false;
    ...
    private SoftReference mWorkspacePreview;
    private long screenId;
    
    private FrameLayout edit_container;
    private ImageView delete_btn;
    private ImageView add_btn;
    public boolean mIsAddCellLayout;
    public boolean isAutoAdded = false;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mOverScrollForegroundDrawable = res.getDrawable(R.drawable.overscroll_glow);
        mForegroundPadding = res.getDimensionPixelSize(R.dimen.workspace_overscroll_drawable_padding);
        mWorkspacePaddingBottom = res.getDimensionPixelSize(R.dimen.hotseat_plus_indicator_height);
        mWorkspacePaddingTop = res.getDimensionPixelSize(R.dimen.notification_bar_height);
        mReorderHintAnimationMagnitude = (REORDER_HINT_MAGNITUDE * IconCache.getAppIconSize(res));
    ...
    public void setEditMode(boolean start) {
        if (start) {
            ...
            } else if (mShortcutsAndWidgets.getChildCount() == 0) {
    ...
    public void updateEditStatus(int childCount) {
        if (!mLauncher.isEditMode() || mLauncher.isFloating()
                || !(getParent() instanceof Workspace)) {
            return;
        }
        edit_container.setVisibility(View.VISIBLE);
        if (childCount == 0) {
    ...
    public float getChildrenScale() {
        if (mLauncher.isEditMode() && !mLauncher.isFloating()) {
            return EditModeUtils.SCALE;
        }
        return 1.0f;
    }

    public float getChildrenScale(View view) {
        boolean isFloated = mLauncher.getFloating().isFloatedCell(view);
        if (isFloated || view instanceof AppWidgetHostView) {
            return 1f;
        }
        float scale = mLauncher.isFloating() ? FloatingLayer.FLOATING_SCALE : getChildrenScale();
        return scale;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mFolderOuterRings.size(); i++) {
            ...
            canvas.save();
            canvas.translate(centerX - width / 2, centerY - height / 2 - 15);  // 15 is the height of app title
    ...
    public boolean isInvalidCell(LayoutParams params) {
        if (params.cellX + params.cellHSpan > mCountX
                || params.cellY + params.cellVSpan > mCountY) {
            return true;
        }
        return false;
    }

    public boolean addViewToCellLayoutTemp(View child, int index, LayoutParams params) {
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        child.setScaleX(getChildrenScale(child));
        child.setScaleY(getChildrenScale(child));
...
/*        if (mLauncher.isEditMode()) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // Begin, here recalulate the cell width and cell height in order to reduce the gap to 0, zhumeiquan
        mCellWidth  = (widthSpecSize - getPaddingLeft() - getPaddingRight()) / mCountX;
        mCellHeight = (heightSpecSize - getPaddingTop() - getPaddingBottom()) / mCountY;
        // End
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mForegroundRect.set(mForegroundPadding, mWorkspacePaddingTop,
                w - mForegroundPadding, h - mForegroundPadding - mWorkspacePaddingBottom);
    }
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            va.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    float scale = getChildrenScale(child);
    ...
    class ReorderHintAnimation {
        ...
        public ReorderHintAnimation(View child, int cellX0, int cellY0, int cellX1, int cellY1,
            ...
            finalScale = getChildrenScale(child) - 4.0f / child.getWidth();
        ...
        void animate() {
            ...
            va.addListener(new AnimatorListenerAdapter() {
                public void onAnimationRepeat(Animator animation) {
                    ...
                    initDeltaX = 0;
                    initDeltaY = 0;
                    initScale = getChildrenScale(child);
                }
            });
        ...
        private void completeAnimationImmediately() {
            ...
            a = s;
            //Add for mix floating drag by fan.yang
//            float scale = mLauncher.isFloating() ? child.getScaleX()
//                    : getChildrenScale();
            float scale = getChildrenScale(child);
            s.playTogether(
                LauncherAnimUtils.ofFloat(child, "scaleX", scale),
                LauncherAnimUtils.ofFloat(child, "scaleY", scale),
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    /**
     * Find the vacant cell starting from last occupied cell, if there is one.
     *
     * @param vacant Holds the x and y coordinate of the vacant cell
     * @param spanX Horizontal cell span.
     * @param spanY Vertical cell span.
     ...
     * @return True if a vacant cell was found
     */
    public boolean getLastVacantCell(int[] vacant, int spanX, int spanY) {

        return findLastVacantCell(vacant, spanX, spanY, mCountX, mCountY, mOccupied);
    }
    
    public static boolean findLastVacantCell(int[] vacant, int spanX, int spanY,
            int xCount, int yCount, boolean[][] occupied) {

    	int startX = 0, startY = 0;
    	boolean foundCell = false, locked = false;
    	// find the last start.
    	...
    	// if span == 1, we have find it, just return.
    	if(spanX == 1 && spanX == 1){
    		vacant[0] = startX;
    		vacant[1] = startY;
    		return true;
    	}
    	// find cell from startX and startY
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public void markWidgetLineOccupied(boolean value,LauncherAppWidgetInfo launcherAppWidgetInfo){
        int y = launcherAppWidgetInfo.cellY;
        int spanY = launcherAppWidgetInfo.spanY;
        if(y < 0) return ;
        if(value){
            markCellsForView(0,y,mCountX,spanY,mOccupied,value);
        }else{
            for(int i = 0 ; i < mCountX ; i++){
                for(int j = y ; j < y + spanY && j < mCountY ; j ++ ){
                    mOccupied[i][j] = getChildAt(i,j) != null;
                }
            }
        }
    }
...
//            throw new RuntimeException("Position exceeds the bound of this CellLayout");
    ...
    // Begin, added by zhumeiquan, 20130716
    ...
    public void onClick(View v) {
        ...
        int screenIndex = ((ViewGroup) getParent()).indexOfChild(this);
        if (screenIndex != workspace.getCurrentPage()) {
            return;
        }
        /*switch (v.getId()) {
        case R.id.delete_btn:
            workspace.removeScreen(screenIndex);
            for (int i = 0; i < workspace.getChildCount(); i++) {
                CellLayout cl = (CellLayout) workspace.getChildAt(i);
                workspace.resetScreenParams(cl);
            }
            workspace.updateEditStatus(true, workspace.getChildCount());
        	break;
        case R.id.add_btn:
            CellLayout layout = workspace.addScreen(screenIndex);
            if (layout != null) {

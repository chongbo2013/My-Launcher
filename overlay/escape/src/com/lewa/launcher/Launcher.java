...
//    private FolderOpenView openFolderBgRight;
	...
	private static final String CHANNEL = "LAUNCHER_TCL";
	private static final boolean BI_OPEN = true;
    // end
...
//        }
        ...
        initDisplay();
		if(BI_OPEN){
		   BIAgent.setLewaPartner(this, CHANNEL);
		}
...
//    }
    ...
    private void showAppRecommend() {
        ...
        if (info.recommendApps.size() > 0 && mAppRecommend.getParent() == null) {
    ...
    private void hideAppRecommend() {
        ...
        if (mAppRecommend.getParent() != null) {
...
//               PreferencesProvider.putStringValue(this,EditModeUtils.PREFERENCE_WALLPAPER_PATH, files[0].getAbsolutePath());
	...
	/**
     * @author jiangyulong 2013-4-2 ,
     */
    public void collectDataForBI() {
        SharedPreferences sp = PreferencesProvider.getSharedPreferences(this);
    	if (mOlderTime == 0) {
    	    mOlderTime = sp.getLong(EFFECTIVE_TIME, 0);
    	}
    	
    	if (System.currentTimeMillis() - mOlderTime  <= DATA_TIME) {
    	    return ;
    	}
    	mOlderTime = System.currentTimeMillis() ;
        sp.edit().putLong(EFFECTIVE_TIME, mOlderTime).commit();
        final ContentResolver cr = getContentResolver();
        new Thread(new Runnable() {
            @Override
            public void run() {
                final Cursor c = cr.query(LauncherSettings.Favorites.CONTENT_URI, null, null, null, null);
                final StringBuilder sb_bottomShortcuts = new StringBuilder();
                final StringBuilder sb_Shortcuts = new StringBuilder();
                final StringBuilder sb_Widgets = new StringBuilder();
                final StringBuilder sb_WidgetsLocation = new StringBuilder();
                int shortcut_num = 0 ;
                try {
                     ...
                     int container, itemType, screen, cellX, cellY, spanX, spanY;
                     while (c.moveToNext()) {
                         boolean isAdd = false ;
                         container = c.getInt(containerIndex);
                         String intentPackageName = null;
                         try {
                             intentPackageName = getIntentPackageName(c.getString(intentIndex));
                         } catch(Exception e) {
                             Log.w(TAG, "Desktop items loading interrupted:", e);
                         }
                         
                         if (intentPackageName != null && intentPackageName.length() > 0) {
                             isAdd = true;
                         }
                         ...
                         case LauncherSettings.Favorites.CONTAINER_DESKTOP:
                             ...
                             case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                 if (isAdd) {
                                     shortcut_num++;
                                     sb_Shortcuts.append(intentPackageName).append(",");
                                 }
                                 break;
                             case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET :
                                 if (isAdd) {
                                     sb_Widgets.append(intentPackageName).append(",");
                                     ...
                                     sb_WidgetsLocation.append(intentPackageName).append("|").
                                         append(screen).append("|").
                                         append(cellX).append("|").
                                         append(cellY).append("|").
                                         append(spanX).append("|").
                                         append(spanY).append("|").append(",");
                                 }
                                 break;
                             }
                             break;
                         case LauncherSettings.Favorites.CONTAINER_HOTSEAT :
                             if (isAdd) {
                                 sb_bottomShortcuts.append(intentPackageName).append(",");
                             }
                             break;
                         }
                     }
                ...
                deleteCharAtLaste(sb_bottomShortcuts);
                deleteCharAtLaste(sb_Shortcuts );
                deleteCharAtLaste(sb_Widgets );
                deleteCharAtLaste(sb_WidgetsLocation);
                HashMap<String, String> purchase= new HashMap<String, String>();
                purchase.put("screen_num", String.valueOf(LauncherModel.getScreenInfoMap().size()));
                purchase.put("installed_widgets", getInstalledWidgets());
                purchase.put("used_widgets", sb_Widgets.toString());
                purchase.put("widget_location", sb_WidgetsLocation.toString());
                purchase.put("shortcut_num", String.valueOf(shortcut_num));
                purchase.put("schortcuts", sb_Shortcuts.toString());
                purchase.put("bottom_shortcuts", sb_bottomShortcuts.toString());
                try {
                    BIAgent.onEvent(Launcher.this, "screen", purchase);
                } catch(Throwable t) {  
